...
class Player {
  ...
  update(deltaTime, keys, width, height) {
    ...
    if (turningLeft) this.angle -= this.rotationSpeed * deltaTime;
    if (turningRight) this.angle += this.rotationSpeed * deltaTime;
    ...
    if (this.redFlash > 0) this.redFlash -= deltaTime;
  ...
  shoot(gameTime) {
    if (gameTime - this.lastShot < this.fireRate) return null;
  ...
  draw(ctx, keys) {
    ...
    if (accelerating) this.drawEngineFlame(ctx);
  ...
  drawAttachedParasites(ctx) {
    for (const parasite of this.attachedParasites) {
      ...
      if (parasite.sprite.loaded) {
        ...
        const h = w * (parasite.sprite.height / parasite.sprite.width);
        ctx.drawImage(parasite.sprite, -w / 2, -h / 2, w, h);
      } else {
...
class Asteroid {
  constructor(x, y, radius, wave) {
    ...
    this.radius = radius;
    this.wave = wave;
    this.sprite = AssetLoader.random(IMAGE_ASSETS.asteroids);
  ...
  draw(ctx) {
    ...
    if (this.sprite.loaded) {
      ...
      ctx.fillStyle = "rgba(217, 227, 255, 0.16)";
...
class SpaceDebris {
  constructor(width, height) {
    ...
    this.radius = Utils.randomRange(54, 126);
    this.x = Utils.randomRange(-width * 0.2, width * 1.2);
    this.y = Utils.randomRange(-height * 0.2, height * 1.2);

    const angle = Utils.randomRange(0, Math.PI * 2);
    const speed = Utils.randomRange(8, GAME_CONFIG.debris.phaseOneDriftSpeed);

    this.vx = Math.cos(angle) * speed;
    this.vy = Math.sin(angle) * speed;
    this.angle = Utils.randomRange(0, Math.PI * 2);
    this.rotation = Utils.randomRange(-0.18, 0.18);
    this.depth = Utils.randomRange(0.55, 1.15);
  }
  ...
  draw(ctx) {
    ...
    ctx.filter = "brightness(97%)";

    if (this.sprite.loaded) {
      const size = this.radius * 3.12 * this.depth;
      ctx.drawImage(this.sprite, -size / 2, -size / 2, size, size);
    } else {
      ctx.strokeStyle = "rgba(185, 230, 255, 0.3)";
...
class CentipedeCreature {
  ...
  update(deltaTime, width, height, player) {
    ...
    if (this.attached) {
      ...
      if (this.lostTimer <= 0) this.dead = true;
      return;
    }
    ...
    this.angle += Utils.clamp(
      angleDiff,
      -GAME_CONFIG.centipede.turnSpeed * deltaTime,
      GAME_CONFIG.centipede.turnSpeed * deltaTime
    );
