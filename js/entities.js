...
class SpaceDebris {
  constructor(width, height) {
    ...
    this.vy = Math.sin(angle) * speed;
    this.angle = Utils.randomRange(0, Math.PI * 2);
    this.rotation = Utils.randomRange(-0.12, 0.12);
...
class EnemyShip {
  constructor(width, height, side = 1) {
    ...
    this.x = side < 0 ? -80 : width + 80;
    this.y = Utils.randomRange(height * 0.22, height * 0.78);
    this.radius = config.radius;
    this.health = config.health;
    this.dead = false;
    this.side = side;
    this.sprite = AssetLoader.get(IMAGE_ASSETS.enemyShips[0]);
    this.angle = side < 0 ? 0 : Math.PI;
    this.phase = Utils.randomRange(0, Math.PI * 2);
    this.speed = config.speed;
    this.fireCooldown = Utils.randomRange(0.35, 0.95);
    this.driftSign = side;
  }
  ...
  update(deltaTime, width, height, player) {
    ...
    const orbitAngle = this.phase + (this.side < 0 ? 0 : Math.PI);
    ...
    const driftX = -Math.sin(orbitAngle) * 60 * this.driftSign;
    const driftY = Math.cos(orbitAngle) * 42;
