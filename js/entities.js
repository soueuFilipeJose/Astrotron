...
class SpaceDebris {
  constructor(width, height) {
    ...
    this.radius = Utils.randomRange(18, 42);
    this.x = Utils.randomRange(-width * 0.2, width * 1.2);
    this.y = Utils.randomRange(-height * 0.2, height * 1.2);

    const angle = Utils.randomRange(0, Math.PI * 2);
    const speed = Utils.randomRange(9, GAME_CONFIG.debris.phaseOneDriftSpeed);
  ...
  draw(ctx) {
    ...
    if (this.sprite.loaded) {
      const size = this.radius * 3.12 * this.depth * GAME_CONFIG.debris.backgroundScale;
      ctx.filter = "brightness(94%)";
      ctx.drawImage(this.sprite, -size / 2, -size / 2, size, size);
      ctx.filter = "none";
    } else {
...
class CentipedeCreature {
  ...
  update(deltaTime, width, height, player) {
    ...
    this.angle += Utils.clamp(angleDiff, -GAME_CONFIG.centipede.turnSpeed * deltaTime, GAME_CONFIG.centipede.turnSpeed * deltaTime);
