class AstrotronGame {
  ...
  spawnAsteroid(radius, x = null, y = null, spriteSrc = null) {
    let spawnX = x;
    let spawnY = y;
    ...
    this.asteroids.push(new Asteroid(spawnX, spawnY, radius, this.wave, spriteSrc));
  }
  ...
  updateGameplay(deltaTime) {
    ...
    const enemiesCleared = this.asteroids.length === 0 && this.centipedes.length === 0 && this.enemyShips.length === 0;
  ...
  updateEnemyShipSystem(deltaTime) {
    ...
    if (this.phaseTwoEnemyTimer >= GAME_CONFIG.enemyShip.phaseTwoDelay) {
      ...
      this.setMission("Dois caças inimigos cercam a ASTROTRON. Tiros lentos com rastro vermelho detectados.");
    }
  }
