class AstrotronGame {
  ...
  resize() {
    ...
    this.playHeight = Math.max(320, this.height - this.cockpitHeight);
  ...
  preloadPhaseAssets(phase) {
    ...
    [IMAGE_ASSETS.player, IMAGE_ASSETS.nyxPortrait].forEach((src) => src && sources.add(src));
  ...
  updateGameplay(deltaTime) {
    ...
    for (const debris of this.debris) {
      debris.update(deltaTime, this.width, this.playHeight, this.debris);
    }
