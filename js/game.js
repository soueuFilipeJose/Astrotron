class AstrotronGame {
  ...
  updateGameplay(deltaTime) {
    ...
    for (const debris of this.debris) {
      debris.update(deltaTime, this.width, this.height);
    }
  ...
  addXp(amount) {
    this.xp += amount * GAME_CONFIG.progression.xpGainMultiplier;
  ...
  openUpgradeScreen(origin = "levelup") {
    ...
    UPGRADE_OPTIONS.forEach((upgrade) => {
      ...
      card.className = canBuy ? "upgrade-card" : "upgrade-card locked";
      card.innerHTML = `
  ...
  getUpgradeCost(upgrade, currentLevel) {
    const baseCost = upgrade.cost || GAME_CONFIG.economy.upgradeBaseCost;
    return baseCost + currentLevel * GAME_CONFIG.economy.upgradeCostGrowth;
  }
  ...
  applyUpgrade(type, origin) {
    ...
    if (this.score < cost) {
      this.setMission(`Créditos insuficientes. ${upgrade.name} custa ${cost} créditos.`);
      this.openUpgradeScreen(origin);
      return;
    }
