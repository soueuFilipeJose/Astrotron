class AstrotronGame {
  ...
  bindEvents() {
    ...
    window.addEventListener("keydown", (event) => {
      const gameplayKeys = ["Space", "ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight", "KeyW", "KeyA", "KeyS", "KeyD", "Enter"];
      if (gameplayKeys.includes(event.code)) event.preventDefault();

      this.keys[event.code] = true;

      if (event.code === "Enter" && this.state === "menu") this.startGame();
      if (event.code === "Enter" && this.state === "cutscene") this.nextDialogue();
      if (event.code === "KeyR" && this.state === "gameover") this.startGame();
    });
  ...
  spawnPhaseOne() {
    const asteroidAmount = 13;

    for (let i = 0; i < asteroidAmount; i++) {
      const size = i < 6 ? GAME_CONFIG.asteroid.large : GAME_CONFIG.asteroid.medium;
      this.spawnAsteroid(size);
    }

    for (let i = 0; i < GAME_CONFIG.debris.phaseOneAmount; i++) {
      this.debris.push(new SpaceDebris(this.width, this.height));
    }

    this.setMission("Fase 1: Cinturão Orfeu. Muitos asteroides e destroços mecânicos em rota de colisão.");
    this.updateUI();
  }

  spawnPhaseTwo() {
    for (let i = 0; i < 2; i++) this.spawnAsteroid(GAME_CONFIG.asteroid.small);

    for (let i = 0; i < Math.floor(GAME_CONFIG.debris.phaseOneAmount * 0.35); i++) {
      this.debris.push(new SpaceDebris(this.width, this.height));
    }

    for (let i = 0; i < GAME_CONFIG.centipede.phaseTwoAmount; i++) {
      this.centipedes.push(new CentipedeCreature(this.width, this.height, this.wave));
    }

    this.setMission("Fase 2: Enxame Biomecânico. Poucos asteroides. Evite as centopeias: elas grudam no casco.");
    this.updateUI();
  }
  ...
  spawnMixedPhase() {
    ...
    for (let i = 0; i < Math.floor(GAME_CONFIG.debris.phaseOneAmount * 0.55); i++) {
      this.debris.push(new SpaceDebris(this.width, this.height));
    }

    for (let i = 0; i < centipedeAmount; i++) {
      this.centipedes.push(new CentipedeCreature(this.width, this.height, this.wave));
    }
  ...
  spawnAsteroid(radius, x = null, y = null) {
    ...
    if (spawnX === null || spawnY === null) {
      ...
      if (edge === 0) { spawnX = Utils.randomRange(0, this.width); spawnY = -radius; }
      if (edge === 1) { spawnX = this.width + radius; spawnY = Utils.randomRange(0, this.height); }
      if (edge === 2) { spawnX = Utils.randomRange(0, this.width); spawnY = this.height + radius; }
      if (edge === 3) { spawnX = -radius; spawnY = Utils.randomRange(0, this.height); }
    }
  ...
  loop(timestamp) {
    ...
    if (this.state === "playing") this.updateGameplay(deltaTime);
  ...
  updateGameplay(deltaTime) {
    ...
    for (const bullet of this.bullets) bullet.update(deltaTime, this.width, this.height);
    for (const asteroid of this.asteroids) asteroid.update(deltaTime, this.width, this.height);
    for (const debris of this.debris) debris.update(deltaTime, this.width, this.height);
    for (const centipede of this.centipedes) centipede.update(deltaTime, this.width, this.height, this.player);
    ...
    const enemiesCleared = this.asteroids.length === 0 && this.centipedes.length === 0;
    if (enemiesCleared && this.state === "playing") this.completeWave();
    if (this.player.hull <= 0 && this.state === "playing") this.endGame();
    this.updateUI();
  }
    ...
    for (const bullet of this.bullets) {
      for (const asteroid of this.asteroids) {
        ...
        if (Utils.circleCollision(bullet, asteroid)) {
          ...
          if (asteroid.dead) this.destroyAsteroid(asteroid);
        }
      }
    }
  }
  ...
  handleBulletCentipedeCollisions() {
    for (const bullet of this.bullets) {
      for (const centipede of this.centipedes) {
        if (bullet.dead || centipede.dead || centipede.attached) continue;
    ...
    for (const asteroid of this.asteroids) {
      ...
      if (Utils.circleCollision(this.player, asteroid)) {
        ...
        if (tookDamage) {
          ...
          if (this.player.hull <= 0) this.endGame();
        }
      }
    }
  }
  ...
  handlePlayerCentipedeCollisions() {
    for (const centipede of this.centipedes) {
      if (centipede.dead || centipede.attached) continue;
  ...
  destroyAsteroid(asteroid) {
    const scoreGain = Math.round(asteroid.radius * 4.2 + this.wave * 5);
  ...
  openUpgradeScreen(origin = "levelup") {
    ...
    UPGRADE_OPTIONS.forEach((upgrade) => {
      ...
      const cost = this.getUpgradeCost(upgrade.type);
      const canBuy = this.score >= cost;
      ...
      card.innerHTML = `
        ...
        <p><strong>Custo:</strong> ${cost} créditos</p>
      `;
    ...
    if (origin === "station") {
      ...
      this.setMission("Núcleo carregado. Escolha uma melhoria e pague pelos módulos.");
    }
  ...
  createExplosion(x, y, color, power = 1, amount = 12) {
    for (let i = 0; i < amount; i++) this.particles.push(new Particle(x, y, color, power));
  }
  ...
  updateUI() {
    ...
    const hullPercent = this.player ? Utils.clamp((this.player.hull / this.player.maxHull) * 100, 0, 100) : 100;
  ...
  draw() {
    ...
    for (const debris of this.debris) debris.draw(this.ctx);
    for (const bullet of this.bullets) bullet.draw(this.ctx);
    for (const asteroid of this.asteroids) asteroid.draw(this.ctx);
    for (const centipede of this.centipedes) centipede.draw(this.ctx);
    for (const particle of this.particles) particle.draw(this.ctx);
    if (this.player) this.player.draw(this.ctx, this.keys);
  ...
  drawMachineSilhouette() {
    ...
    ctx.strokeStyle = this.wave === 2 ? "rgba(255, 75, 95, 0.38)" : "rgba(185, 230, 255, 0.42)";
    ctx.lineWidth = 1;
    ...
    for (let i = 0; i < 7; i++) {
      const offset = i * 22;
      ctx.strokeRect(-size / 2 + offset, -size / 2 + offset * 0.3, size - offset * 1.4, size - offset * 0.8);
    }
